...
import { DIAGNOSTIC_COPY, type DiagnosticLocale, } from "./diagnostic-copy";
import styles from "./diagnostic.module.css";
type ShareState = "idle" | "submitting" | "success" | "email-error" | "send-error";
...
export function BottleneckDiagnostic({ id = "bottleneck-check", title, intro, className, introOnly = false, locale = "en", }: BottleneckDiagnosticProps) {
    ...
    const primaryContactAction = useMemo(() => getPrimaryContactAction(locale), [locale]);
    ...
    return (<div className={`${styles.diagnostic} ${className ?? ""}`} id={id}>
      ...
      <div className={styles.progressBlock}>
        <p aria-live="polite" className={styles.progressValue} role="status">
          {copy.answered(answeredCount, DIAGNOSTIC_ITEMS.length)}
        </p>
        <div aria-label={copy.progressLabel} aria-valuemax={DIAGNOSTIC_ITEMS.length} aria-valuemin={0} aria-valuenow={answeredCount} aria-valuetext={copy.answered(answeredCount, DIAGNOSTIC_ITEMS.length)} className={styles.progress} role="progressbar">
          <span className={styles.progressFill} style={{
      ...
      <div className={styles.statementBlock}>
        <ol className={styles.statementList} id={`${id}-statements`}>
          {DIAGNOSTIC_ITEMS.map((item, index) => {
            ...
            return (<li className={`${styles.statementItem} ${isCollapsible ? styles.statementCollapsible : ""} ${isCollapsible && expanded
                    ? styles.statementCollapsibleExpanded
                    : ""}`} aria-hidden={isCollapsible && !expanded ? true : undefined} inert={isCollapsible && !expanded} key={item.id}>
                  ...
                  <fieldset className={styles.statementFieldset}>
                    ...
                    <div className={styles.options}>
                      <label className={styles.option}>
                        <input checked={selectedAnswer === true} className={styles.radio} name={`${radioGroupId}-${item.id}`} onChange={() => selectAnswer(item.id, true)} ref={index === 0 ? firstRadioRef : undefined} type="radio" value="true"/>
                        <span>{copy.trueLabel}</span>
                      </label>
                      <label className={styles.option}>
                        <input checked={selectedAnswer === false} className={styles.radio} name={`${radioGroupId}-${item.id}`} onChange={() => selectAnswer(item.id, false)} type="radio" value="false"/>
        ...
        <Button aria-controls={`${id}-statements`} aria-expanded={expanded} className={styles.statementToggle} onClick={() => setExpanded((current) => !current)} size="compact" variant="text">
      ...
      {result ? (<div aria-describedby={`${id}-result-score ${id}-result-body`} aria-labelledby={`${id}-result-title`} className={styles.result} ref={resultRef} role="region" tabIndex={-1}>
          ...
          {shareOpen ? (<div className={styles.sharePanel} id={`${id}-share-form`}>
              {shareState === "success" ? (<div aria-live="polite" className={`${styles.shareNotice} ${styles.shareSuccess}`} role="status">
                  <CheckCircle2 aria-hidden="true" size={19}/>
                  <p>{copy.sent}</p>
                </div>) : (<form className={styles.shareForm} noValidate onSubmit={sendResult}>
                  <p className={styles.shareTitle}>{copy.shareTitle}</p>
                  <div className={styles.emailField}>
                    <label htmlFor={`${id}-result-email`}>{copy.emailLabel}</label>
                    <input aria-describedby={`${id}-email-help${shareState === "email-error" ||
                        shareState === "send-error"
                        ? ` ${id}-share-error`
                        : ""}`} aria-invalid={shareState === "email-error"} autoComplete="email" id={`${id}-result-email`} inputMode="email" maxLength={254} onChange={(event) => {
                        setEmail(event.target.value);
                        if (shareState === "email-error" ||
                            shareState === "send-error") {
                            setShareState("idle");
                        }
                    }} ref={emailRef} required type="email" value={email}/>
                  ...
                  <div aria-hidden="true" className={styles.honeypot}>
                    ...
                    <input autoComplete="off" id={`${id}-result-website`} onChange={(event) => setWebsite(event.target.value)} tabIndex={-1} type="text" value={website}/>
                  </div>
                  {shareState === "email-error" ||
                        shareState === "send-error" ? (<div className={`${styles.shareNotice} ${styles.shareError}`} id={`${id}-share-error`} role="alert">
                  ...
                  <div className={styles.shareFooter}>
                    <Button disabled={shareState === "submitting"} type="submit">
                      {shareState === "submitting" ? (<LoaderCircle aria-hidden="true" className={styles.spinner} size={18}/>) : (<Send aria-hidden="true" size={17}/>)}
