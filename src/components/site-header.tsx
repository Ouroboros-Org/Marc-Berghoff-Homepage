...
import { getHeaderNavigation, getPrimaryContactAction, siteConfig, } from "@/config/site";
...
export function getActiveHeaderGroupId(pathname: string) {
    const navigation = getHeaderNavigation("en");
    const containsPath = (href: string) => pathname === href || pathname.startsWith(`${href}/`);
    return (navigation.find((group) => containsPath(group.href))?.id ??
        navigation.find((group) => group.items.some((item) => containsPath(item.href)))?.id ??
        null);
}
export function isCurrentHeaderItem(pathname: string, groupId: string, href: string) {
    return (getActiveHeaderGroupId(pathname) === groupId &&
        isCurrentNavigationPage(pathname, href));
}
function LinkedInIcon({ size = 20 }: {
    size?: number;
}) {
    return (<svg aria-hidden="true" fill="currentColor" height={size} viewBox="0 0 24 24" width={size}>
...
export function SiteHeader({ locale = "en" }: {
    locale?: SiteLocale;
}) {
    ...
    const openDesktopMenu = desktopMenuState?.pathname === pathname ? desktopMenuState.value : null;
    const openMobileSection = mobileSectionState?.pathname === pathname ? mobileSectionState.value : null;
    ...
    useEffect(() => {
        ...
        const onKeyDown = (event: KeyboardEvent) => {
            ...
            if (event.key === "Tab") {
                const focusable = Array.from(headerRef.current?.querySelectorAll<HTMLElement>('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])') ?? []).filter((element) => element.offsetParent !== null &&
    ...
    return (<header ref={headerRef} className="site-header" onClick={(event) => {
      ...
      <div className="site-header__inner">
        ...
        <nav className="desktop-nav" aria-label={"Primary navigation"}>
          {navigation.map((group) => {
            ...
            return (<div className="desktop-nav__group" key={group.id}>
                <Button aria-controls={`desktop-submenu-${group.id}`} aria-expanded={open} className="desktop-nav__trigger" data-active={active || undefined} data-menu-trigger={group.id} onClick={() => setDesktopMenuState((current) => current?.pathname === pathname && current.value === group.id
                    ? null
                    : { value: group.id, pathname })} onKeyDown={(event) => {
                    ...
                    window.requestAnimationFrame(() => {
                        headerRef.current
                            ?.querySelector<HTMLElement>(`#desktop-submenu-${group.id} a[href]`)
                            ?.focus();
                    });
                }} size="compact" variant="text">
                  {group.label}
                  <ChevronDown aria-hidden="true" className="desktop-nav__chevron" size={15}/>
                </Button>
              </div>);
        })}
        ...
        <ButtonLink className="header-cta" cta href={contactAction.href} size="compact">
          {contactAction.label}
        </ButtonLink>

        <Button aria-controls="mobile-navigation" aria-expanded={mobileOpen} aria-label={mobileOpen
            ?
                "Close navigation"
            :
                "Open navigation"} className="mobile-menu-button" onClick={() => setMobileState((current) => current?.pathname === pathname ? null : { value: "open", pathname })} ref={menuButtonRef} size="icon" variant="ghost">
      ...
      <div aria-hidden={!mobileOpen} className="mobile-nav" data-open={mobileOpen || undefined} id="mobile-navigation">
        <nav className="mobile-nav__inner" aria-label={"Mobile navigation"}>
          ...
          {navigation.map((group, index) => {
            ...
            const active = activeGroupId === group.id;
            return (<div className="mobile-nav__section" key={group.id}>
